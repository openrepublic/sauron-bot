...
async def build_producer_status_message(
    ...
    system_message = (
        ...
        f"<code>{format_fixed_width('Clock:', f'{clock_offset}')}</code>\n"
        f"<code>{format_fixed_width('CPU Load:', f'[ {cpu_load.min_1} {cpu_load.min_5} {cpu_load.min_15} ]')}</code>\n"
        f"<code>{format_fixed_width('RAM Usage:', f'{ram_usage} %')}</code>\n"
        f"<code>{format_fixed_width('Disk Usage:', f'{disk_usage} %')}</code>\n"
        f"<code>{format_fixed_width('Nodeos:', f'{nodeos_status}')}</code>\n"
    )

    network_message = (
        f"<b>Network Information:</b>\n"
        f"<code>{format_fixed_width('Ping:', f'{ping} ms')}</code>\n"
        f"<code>{format_fixed_width('Down:', f'{down} Mbps')}</code>\n"
        f"<code>{format_fixed_width('Up:', f'{up} Mbps')}</code>\n"
        f"<code>{format_fixed_width('Updated at:', network_updated_at)}</code>\n"
    )

    bp_status_message = (
        f"<b>BP Stats:</b>\n"
        f"<code>{format_fixed_width('Is active:', f'{bp_status.is_active}')}</code>\n"
        f"<code>{format_fixed_width('Total votes:', f'{total_votes}')}</code>\n"
        f"<code>{format_fixed_width('Produced blocks:', f'{lifetime_produced_blocks}')}</code>\n"
        f"<code>{format_fixed_width('Missed blocks:', f'{lifetime_missed_blocks}')}</code>\n"
        f"<code>{format_fixed_width('Missed bpr:', f'{missed_blocks_per_rotation}')}</code>\n"
        f"<code>{format_fixed_width('Unpaid blocks:', f'{unpaid_blocks}')}</code>\n"
        f"<code>{format_fixed_width('Payment:', f'{bp_status.payment}')}</code>\n"
        f"<code>{format_fixed_width('Accuracy:', f'{accuracy} %')}</code>\n"
    )
    ...
    if clock_offset != 'Synced' or bp_status.alert or sys_health_check.alert:
        ...
        return response
    response += f"\n<tg-emoji emoji-id='9989'>✅</tg-emoji>"
    return response
...
def get_schedule_message(schedule: list, producer_name):
    msg = f'<b>Schedule:</b>\n'
    for bp in range(1, len(schedule)):
        if schedule[bp] == producer_name:
            msg += f"<code>{bp} - </code><b>{schedule[bp]}</b> <tg-emoji emoji-id='128640'>🚀</tg-emoji>\n"
            continue
        msg += f"<code>{bp} - {schedule[bp]}</code>\n"
    return msg
...
def get_rotation_message(rotation: Rotation):
    msg = (
        f'<b>Rotation:</b>\n'
        f"<code>{format_fixed_width('On schedule:', f'{rotation.active}')}</code>"
    )
    if rotation.active:
        msg += (
            f"\n<code>{format_fixed_width('Prev:', f'{rotation.prev_bp}')}</code>"
            f"\n<code>{format_fixed_width('Next:', f'{rotation.next_bp}')}</code>"
        )
    return msg 
 
def format_fixed_width(key, value):
    total_width = 25 
    key_width = len(key)
...
def build_tags(users_alerted: str | None):
    tags = f"\n<tg-emoji emoji-id='128680'>🚨</tg-emoji>\n"
